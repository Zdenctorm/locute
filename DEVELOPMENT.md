...
**Aktuální Locute = větev `main` na https://github.com/Zdenctorm/locute**

| Co *není* „aktuální“ | Proč |
|---|---|
| DMG z [GitHub Releases](https://github.com/Zdenctorm/locute/releases) | Release vzniká ručně; `main` je často novější |
...
```bash
cd ~/locute
# totéž co: cd /Users/anycoin/locute
```

**Ne** `cd ~/anycoin/locute` — to znamená `/Users/anycoin/anycoin/locute` a ta složka neexistuje.

Najdeš clone:

```bash
ls ~/locute/scripts/install_latest.sh
...
```bash
cd ~
git clone https://github.com/Zdenctorm/locute.git locute
cd locute
...
1. Postaví Release do `dist/Locute.app` (`build_release.sh`)
2. Ukončí běžící Locute
3. Nahradí `/Applications/Locute.app`
...
```bash
LOCUTE_INSTALL_PATH="$HOME/Applications/Locute.app" ./scripts/install_latest.sh
```
...
1. **Nastavení → Soukromí a zabezpečení → Zpřístupnění** — smaž staré položky „Locute“, přidej **aktuální** `/Applications/Locute.app` (v appce Nastavení → Oprávnění uvidíš přesnou cestu).
2. V menu baru **Locute → O Locute** — ověř verzi.
...
./scripts/install_latest.sh --list
# ručně smaž např.:
#   dist/Locute.app (jen artefakt buildu — znovu se vytvoří)
#   ~/Library/Developer/Xcode/DerivedData/.../Locute.app
#   staré DMG rozbalené na Desktopu
```

## Vývoj v Xcode

Otevři `Locute.xcodeproj`. Pro běžné testování funkcí preferuj `install_latest.sh` místo Run z Xcode.
...
grep 'Vložit' Locute/App/AppDelegate.swift
```
...
Bez Metal Toolchainu nejde zkompilovat MLX; Locute na `main` ho pro volitelný post-processing potřebuje v buildu vždy.

## Diagnostika

- Log: `~/Library/Logs/Locute/diagnostics.log`
