...
enum AccessibilityAuditEngine {
    ...
    static func runAndSaveReport(
        ...
        let dictatorFindings = auditDictatorUI(extraMenu: extraMenu)
        ...
        let report = AccessibilityAuditReportBuilder.build(
            context: ctx,
            inventory: inventory,
            dictatorFindings: dictatorFindings + menuFindings,
    ...
    static func current() -> AccessibilitySurfaceInventory {
        AccessibilitySurfaceInventory(surfaces: [
            .init(
                ...
                voiceOverNotes: "Status item má dynamický label dle DictatorState; oznámení stavu přes AccessibilitySupport.announce.",
                manualChecks: [
...
enum AccessibilityAuditReportBuilder {
    static func build(
        ...
        dictatorFindings: [AccessibilityAuditFinding],
        ...
        let critical = dictatorFindings.filter { $0.severity == .critical }
        let warnings = dictatorFindings.filter { $0.severity == .warning }
        let infos = dictatorFindings.filter { $0.severity == .info }
        ...
        lines.append("")
        lines.append(referenceComparisonTable(dictatorFindings: dictatorFindings, snapshots: referenceSnapshots))
        lines.append("")
    ...
    static func referenceComparisonTable(
        ...
        let dictatorElements = max(1, dictatorFindings.count + 20)
        let dictatorLabeled = dictatorFindings.filter { !$0.issue.contains("bez label") }.count
        let dictatorLabelRatio = Double(dictatorLabeled) / Double(dictatorElements)
