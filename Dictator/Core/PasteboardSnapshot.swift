...
/// Captures `NSPasteboard.general` before Dictator overwrites it for Cmd+V injection.
struct PasteboardSnapshot: Sendable {
    ...
    ///
    /// - Parameter maxChangeCount: Upper bound on `pasteboard.changeCount` to still treat as "only Dictator touched it".
    @MainActor
    @discardableResult
    ...
    /// After capture, Dictator typically performs `clearContents` + `setString` (+ optional reads).
